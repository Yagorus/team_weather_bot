  ...
  environment = [
     {
       ...
       value = "postgresql://${data.aws_ssm_parameter.password_db.value}:${aws_db_instance.db_instance.username}@${aws_db_instance.db_instance.address}:5432/${aws_db_instance.db_instance.name}"
     }
  ]
}
])
